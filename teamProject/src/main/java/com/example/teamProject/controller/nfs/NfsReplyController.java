...
import com.example.teamProject.service.nfs.NfsReplyService;

@RestController //@ResponseBody瑜� 遺숈씠吏� �븡�븘�룄 酉곌� �븘�땶 �뜲�씠�꽣 由ы꽩 媛��뒫
//spring 4.0遺��꽣 �궗�슜 媛��뒫
@RequestMapping("reply_nfs/*")
public class NfsReplyController {
	@Inject
	NfsReplyService replyService;
	
	@RequestMapping("insert.do")
	public void insert(NfsReplyDTO dto, HttpSession session) {
	...
	public ModelAndView list(int bno, ModelAndView mav) {
		List<NfsReplyDTO> list=replyService.list(bno);
		mav.setViewName("nfs/reply_nfs"); //酉곗쓽 �씠由�
	...
	public List<NfsReplyDTO> list_json(int bno){
