...
import com.example.teamProject.service.fif.FifReplyService;

@RestController //@ResponseBody瑜� 遺숈씠吏� �븡�븘�룄 酉곌� �븘�땶 �뜲�씠�꽣 由ы꽩 媛��뒫
//spring 4.0遺��꽣 �궗�슜 媛��뒫
@RequestMapping("reply_fif/*")
public class FifReplyController {
	@Inject
	FifReplyService replyService;
	
	@RequestMapping("insert.do")
	public void insert(FifReplyDTO dto, HttpSession session) {
	...
	public ModelAndView list(int bno, ModelAndView mav) {
		List<FifReplyDTO> list=replyService.list(bno);
		mav.setViewName("fif/reply_fif"); //酉곗쓽 �씠由�
	...
	public List<FifReplyDTO> list_json(int bno){
