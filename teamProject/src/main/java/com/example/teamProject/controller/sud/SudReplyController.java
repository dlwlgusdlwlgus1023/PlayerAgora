...
import com.example.teamProject.service.sud.SudReplyService;

@RestController //@ResponseBody瑜� 遺숈씠吏� �븡�븘�룄 酉곌� �븘�땶 �뜲�씠�꽣 由ы꽩 媛��뒫
//spring 4.0遺��꽣 �궗�슜 媛��뒫
@RequestMapping("reply_sud/*")
public class SudReplyController {
	@Inject
	SudReplyService replyService;
	
	@RequestMapping("insert.do")
	public void insert(SudReplyDTO dto, HttpSession session) {
	...
	public ModelAndView list(int bno, ModelAndView mav) {
		List<SudReplyDTO> list=replyService.list(bno);
		mav.setViewName("sud/reply_sud"); //酉곗쓽 �씠由�
	...
	public List<SudReplyDTO> list_json(int bno){
