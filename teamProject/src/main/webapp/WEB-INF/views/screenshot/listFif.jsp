...
body{
	background-image: url("../images/fif2.jpg");
...
$(function(){
	$("#btnWrite").click(function(){
		...
		location.href="${path}/teamProject/screenshot/writeFif.do";
		} else {
...
function list(page){
	location.href="${path}/teamProject/screenshot/listFif.do?curPage="+page;
	...
	<%@ include file="../include/headerBody.jsp" %>
		<div class="container-fluid text-center">
			...
			<div id="headbtn">
				<table>
					<tr>
						<td><a href="${path}/fif/list1.do">피파온라인</a></td>
						<td><a href="${path}/fif/list2.do">패치노트</a></td>
						<td><a
							href="${path}/fif/list3.do">공략/팁</a></td>
						<td><a href="${path}/fif/list4.do">파티모집</a></td>
						<td style="background-color: gray;"><a
							href="${path}/screenshot/listFif.do">스크린샷</a></td>
					</tr>
				</table>
			</div>
				...
				<div class="col-md-9 text-left" id="screen">
						<form name="form1" method="post" action="${path}/screenshot/listFif.do">
						
<!-- 검색폼 -->
<table style="background-color: rgba( 255, 255, 255, 0.9 ); width: 100%; max-width: 1054px; margin-left: auto;margin-right: auto;">
<tr>

	<th style="text-align:right ; text-align: right; width: 150px; min-width: 150px;padding: 2px 0px 2px 10px;">
	<select name="search_option" class= "form-control" style="width: 170px;">
		<option value="nick"
...
<c:if test="${map.search_option == 'title'}">selected</c:if>
		>제목</option>
		<option value="posts" 
<c:if test="${map.search_option == 'posts'}">selected</c:if>
		>내용</option>
		<option value="all" 
<c:if test="${map.search_option == 'all'}">selected</c:if>
		...
		<th style="text-align: right; width: 320px; min-width: 320px; padding: 4px 10px 0px 10px;">
			<fieldset><input type="search" name="keyword" class ="search1" placeholder="검색어 입력"/>
	<button type="submit" class ="search2"><i class="fa fa-search" aria-hidden="true"></i></button></fieldset>
</th>
<th style="text-align: left;  ">
	...
	<c:forEach var="row" items="${map.list}" >
		...
		margin: 25px; ">
		<div style="position : relative; vertical-align : middle; line-height : 200px;" onclick="location.href='${path}/screenshot/viewFif.do?bno=${row.bno}';">
		...
		border: 1px solid gray; overflow: hidden;" onclick="location.href='${path}/screenshot/viewFif.do?bno=${row.bno}';">
