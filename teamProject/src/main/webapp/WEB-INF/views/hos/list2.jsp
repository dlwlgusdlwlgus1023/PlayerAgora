...
<script>
	$(function() {
		$("#btnWrite").click(function() {
			location.href = "${path}/hos/write2.do";
		});
	});
	function list(page) {
		location.href = "${path}/hos/list2.do?curPage=" + page;
	}
</script>
</head>
<body>
				...
				<table>
					<tr>
						...
						<td style="background-color: gray;"><a
							href="${path}/hos/list2.do">패치노트</a></td>
						<td><a href="${path}/hos/list3.do">공략/팁</a></td>
						<td><a href="${path}/hos/list4.do">파티모집</a></td>
						<td><a href="${path}/screenshot/listHos.do">스크린샷</a></td>
					</tr>
				</table>
			</div>
			<div class="col-md-9 text-left">

				<form id="form1" method="post" action="${path}/hos/list2.do">
						...
						<tr id="bo_style">
							<c:choose>
								<c:when test="${row.b_name=='패치노트'}">
									<td>${row.bno}</td>
									<td>${row.b_name}</td>
									<td><a href="${path}/hos/view.do?bno=${row.bno}">${row.title}</a>
										<c:if test="${row.cnt > 0}">
