...
<script>
	$(function() {
		$("#btnWrite").click(function() {
			location.href = "${path}/mps/write2.do";
		});
	});
	function list(page) {
		location.href = "${path}/mps/list2.do?curPage=" + page;
	}
</script>
</head>
<body>
	<div style="height: 100%; width: 100%">
		...
		<div class="container-fluid text-center">
				...
				<table>
					<tr>
						...
						<td style="background-color: gray;"><a
							href="${path}/mps/list2.do">패치노트</a></td>
			...
			<div class="col-md-9 text-left">
				...
				<form id="form1" method="post" action="${path}/mps/list2.do">
						...
						<tr id="bo_style">
							<c:choose>
								<c:when test="${row.b_name=='패치노트'}">
									<td>${row.bno}</td>
									<td>${row.b_name}</td>
									<td><a href="${path}/mps/view.do?bno=${row.bno}">${row.title}</a>
										<c:if test="${row.cnt > 0}">
